...
JSON files can be quite big and the format might be compacted. This gives Emacs a bit of trouble, so the following function runs the JSON file through the ~jq~ utility before opening it. The best available binding I could find was ~<C-x C-g>~ because of its proximity to the normal ~find-file~ binding. I might change it
later. This is not yet very safe.

#+begin_src emacs-lisp
  (defun lorcab/pre-open-json-file (file)
    (interactive "fFind JSON file: ")
    (let ((tmp (make-temp-file "jq")))
      (shell-command-to-string (concat "cat " file " | jq > " tmp))
  ...
  (global-set-key
   (kbd "C-x C-g") (lambda ()
                     (interactive)
                     (call-interactively 'lorcab/pre-open-json-file)))
#+end_src
**  REST Client
...
[[https://github.com/zevlg/telega.el][telega.el]] is a Telegram client for Emacs.
...
* To Do
** org-fc [0/1]
- [ ] make sure all org-fc buffers get their text scale reset when a review is over, not just the active one
** json-mode [1/3]
- [X] pipe JSON files through ~jq~ before loading them into the buffer to prevent issues with very long lines
- [ ] ~lorcab/pre-open-json-file~: make a backup copy of the original before piping through ~jq~
- [ ] ~lorcab/pre-open-json-file~: make sure file is JSON before piping through ~jq~
* References
